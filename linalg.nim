...
type
  ...
  Matrix*[M, N: static[int]] = object
    order: OrderType
    data {.align(16).}: ref array[M * N, float]
...
proc `*`*[M, N, K](a: Matrix[M, K], b: Matrix[K, N]): Matrix[M, N] {.noinline.} =
  assert a.order == b.order
  result.order = a.order
  new result.data
  let
    a_data {.noalias.} = cast[ref DoubleArray[K, M]](a.data)
    b_data {.noalias.} = cast[ref DoubleArray[N, K]](b.data)
    res_data {.noalias.} = cast[ref DoubleArray[N, M]](result.data)
  for i in 0 ..< M:
    for j in 0 ..< N:
      for k in 0 ..< K:
